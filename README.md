...
[orderproperties](https://docs.mql4.com/constants/tradingconstants/orderproperties)
...
L'id étant l'id de la __demande correspondante.__
...
`CMD`                       String                  Opération commerciale `Ouverture d'ordre`
...
`STOPLOSS`                  Double                  Valeur littérale ou modificateur (# prix-modificateurs) pour le niveau de stop loss.
`TAKEPROFIT`                Double                  Valeur littérale ou modificateur (# prix-modificateurs) pour le niveau de take profit.
...
`236|ORDER_OPEN|USDJPY|OP_BUY|1|108.848|0|0|0|message de commentaire affiché ici|123|0`

#### Paramètres de la réponse

`TICKET`                    Int                     Billet de commande reçu du serveur de commerce
...
`312|ORDER_MODIFY|140602286|108,252|107,525|109,102`
...
`318|PENDING_ORDER_DELETE|140602286`
...
`345|PENDING_ORDER_DELETE_ALL|USDJPY`
...
`380|MARKET_ORDER_CLOSE|140612332`
...
`383|MARKET_ORDER_CLOSE_ALL|USDJPY`

#### Valeurs de réponse

`DELETED_COUNT`             Int                     Nombre de commandes en attentes supprimées
...
`ORDRE`                     String                  Les commandes avec des valeurs séparées par des virgules `,`.
`TICKET`                    Int                     Billet de commande.
`OPEN_TIME`                 Int                     Commande prix ouvert.
`TYPE`                      [type d'ordre]          Opérations commerciales.
`LOTS`                      Int                     Volume de commande.
`SYMBOL`                    Symbole                 Symbole de commande.
`OPEN_PRICE`                Int                     Commande prix ouvert.
...
`BID`                       Double                  Prix actuel de l'enchère
`ASK`                       Double                  Prix actuel demandé
`SYMBOL`                    Symbole                 Le symbole
...
`DEVISE`                    String                  Devise du compte.
`BALANCE`                   Double                  Solde du compte dans la devise du dépôt.
`PROFIT`                    Double                  Bénéfice courant d'un compte dans la devise de dépôt.
`EQUITY_MARGIN`             Double                  Fonds propres du compte dans la devise du dépôt.
`MARGIN_FREE`               Double                  Marge libre d'un compte dans la devise de dépôt.
`MARGIN_LEVEL`              Double                  Niveau de marge du compte en pourcentage.
`MARGIN_SO_CALL`            Double                  Niveau d'appel de marge.
`MARGIN_SO_SO`              Double                  Niveau d'arrêt de marge.
